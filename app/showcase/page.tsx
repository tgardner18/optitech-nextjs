...
const COLORS = [
  ...
  },
  {
    ...
    modeFixed: false,
    usage: "Primary text and content. High contrast against canvas in both modes. Also used for UI chrome (hamburger lines, borders).",
    border: false,
  },
  {
    token: "--ot-fg-muted",
  ...
  },
  {
    ...
    twBg: "bg-accent",
    name: "Accent",
    darkValue:  "oklch(68% 0.20 68)",
    lightValue: "oklch(68% 0.20 68)",
    modeFixed: true,
    usage: "Amber/saffron accent. Badges, highlights, secondary CTAs, and moments that need warm contrast against teal.",
    border: false,
  },
  {
    ...
    name: "Accent Hover",
    darkValue:  "oklch(56% 0.18 68)",
    lightValue: "oklch(56% 0.18 68)",
    modeFixed: true,
    usage: "Deeper amber for hover and active states on accent-colored surfaces.",
    border: false,
  },
  {
    ...
    lightValue: "oklch(12% 0.012 195)",
    modeFixed: true,
    usage: "Dark text and icons on amber surfaces. Amber at 68% lightness requires dark foreground for contrast.",
    border: false,
  },
] as const;
    ...
    <div className="min-h-screen">
      ...
      <section className="bg-brand px-md pt-xl pb-lg lg:px-lg">
        ...
        <p className="text-body leading-body text-fg-on-brand/80 mt-md max-w-[60ch]">
          A living reference for design tokens, components, and patterns.
          New samples appear here as the library grows. Toggle dark / light mode
          using the button in the header to see all tokens in both modes.
        </p>
      </section>
      ...
      <div className="flex">
        ...
        <aside className="hidden lg:block w-52 shrink-0 border-r border-fg/10">
          <nav className="sticky top-16 p-lg" aria-label="Showcase sections">
            <p className="text-label tracking-label uppercase text-fg-muted mb-md font-semibold">
              Sections
            </p>
            <ul className="flex flex-col gap-xs">
              {NAV_SECTIONS.map((s) => (
                <li key={s.href}>
                  <a
                    href={s.href}
                    className="block text-sm font-normal text-fg-muted py-xs
        ...
        <div className="flex-1 min-w-0 divide-y divide-fg/10">
          ...
          <section id="colors" className="px-md py-xl lg:px-lg">
            ...
            <p className="text-body leading-body text-fg-muted mb-lg max-w-[65ch]">
              Tokens use semantic role names so any theme can be applied by replacing
              the values in <Token name="styles/tokens.css" /> without touching component code.
              Swatches below reflect the active mode — toggle dark / light to compare.
            </p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-lg">
              {COLORS.map((c) => (
                <div key={c.token}>
                  <div
                    className={`h-20 w-full ${c.twBg} ${
                      c.border ? "border border-fg/15" : ""
                    }`}
                  />
                  <div className="mt-sm space-y-xs">
                    <div className="flex items-center gap-sm flex-wrap">
                      <p className="text-title font-semibold leading-title text-fg">
                        {c.name}
                      </p>
                      {c.modeFixed && (
              ...
              {TYPE_SCALE.map((t) => (
                <div key={t.label}>
                  <div className="flex flex-wrap items-baseline gap-x-md gap-y-xs mb-md">
                    ...
                    <span className="font-mono text-label text-fg-muted/60">
                      {t.meta}
                    </span>
                  </div>
                  <p className={`text-fg ${t.classes}`}>
                    {t.sample}
                  </p>
                  <div className="mt-sm">
                    <Token name={t.token} />
            ...
            <div className="mt-xl border-t border-fg/10 pt-lg">
              ...
              <div className="bg-brand p-lg">
                ...
                <p className="text-body leading-body text-fg-on-brand/80 mt-md max-w-[55ch]">
                  Body copy on the brand surface uses <code className="font-mono text-fg-on-brand">fg-on-brand</code> — always
                  light, regardless of mode, because the brand teal requires high-contrast foreground.
                </p>
              </div>
            </div>
          ...
          <section id="buttons" className="px-md py-xl lg:px-lg">
            <SectionLabel index="03 · Components" title="Buttons" />
            <div className="flex flex-col gap-lg">
              ...
              <Ground label="On Brand" className="bg-brand">
                <div className="flex flex-col gap-sm items-start">
                  <button
                    type="button"
                    className="bg-brand-hover hover:bg-canvas
                               text-fg-on-brand text-label font-semibold tracking-label uppercase
                               px-12 py-4
                               transition-colors duration-150 ease-quick
          ...
          <section id="inputs" className="px-md py-xl lg:px-lg">
            <SectionLabel index="04 · Components" title="Form Elements" />
              ...
              <div>
                ...
                <div className="bg-surface p-md">
                  ...
                  <input
                    ...
                    readOnly
                    className="w-full bg-canvas border border-fg/15
                               text-fg placeholder:text-fg-muted
                               text-body font-normal rounded-input
                               px-md py-sm outline-none"
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="bg-surface p-md">
                  ...
                  <input
                    ...
                    readOnly
                    className="w-full bg-canvas border border-brand
                               text-fg
                               text-body font-normal rounded-input
                               px-md py-sm outline-none"
                  />
                  <p className="text-label text-fg-muted/60 mt-xs">
                    Border shifts to brand. No glow.
                  </p>
                </div>
              </div>
            ...
            <p className="text-label text-fg-muted leading-body mt-lg">
              ...
              All other components use sharp corners (0px).
            </p>
          </section>
              ...
              {SPACING.map((s) => (
                <div key={s.token} className="flex items-center gap-md">
                  <div className="w-12 shrink-0">
                    <span className="text-label tracking-label uppercase text-fg font-semibold">
                      {s.name}
                    </span>
                  </div>
                  <div
                    className="h-5 bg-brand shrink-0"
                    style={{ width: s.value }}
                  />
                  <div className="flex items-center gap-sm flex-wrap">
              ...
              {MOTION.map((m) => (
                <div key={m.token} className="bg-surface p-lg">
                  <div className="flex items-baseline gap-md mb-sm">
                    <p className="text-label tracking-label uppercase text-brand font-semibold">
                      {m.label}
                    </p>
