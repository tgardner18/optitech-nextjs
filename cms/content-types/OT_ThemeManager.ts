...
export const OT_ThemeManager = contentType({
  ...
  properties: {
    ...
    peeriusScriptUrl: {
      type: 'string',
      maxLength: 200,
      displayName: 'Product Recommendations Script URL',
      description: 'Full https:// URL of the Optimizely Product Recommendations (Peerius) tracking script (e.g. https://cdn.peerius.episerver.net/{customerId}/js/peerius.thispage.js). When set, product recommendations tracking is injected and the Product Recommendations block renders live recs.',
      group: 'OT_Integrations',
      sortOrder: 40,
    },
    contentRecsApiKey: {
      type: 'string',
      maxLength: 100,
      displayName: 'Content Recommendations API Key',
      description: 'Optimizely Content Recommendations (Idio) delivery API key. Used server-side by the Content Recommendations block to fetch personalized content for the current visitor.',
      group: 'OT_Integrations',
      sortOrder: 50,
    },
    contentRecsClientId: {
      type: 'string',
      maxLength: 64,
      displayName: 'Content Recommendations Client ID',
      description: 'Idio tracking client ID. When set together with the Delivery ID, the Content Recommendations tracking script (ia.js) is injected to build the visitor profile that personalizes recommendations.',
      group: 'OT_Integrations',
      sortOrder: 60,
    },
    contentRecsDeliveryId: {
      type: 'string',
      maxLength: 20,
      displayName: 'Content Recommendations Delivery ID',
      description: 'Idio tracking delivery ID (numeric). Paired with the Client ID to inject the Content Recommendations tracking script.',
      group: 'OT_Integrations',
      sortOrder: 70,
    },
  },
