...
function TypeBadge({ type, className = '' }: { type: string; className?: string }) {
  return (
    <span className={`inline-flex items-center rounded-tl-ot-surface bg-accent text-fg-on-accent px-2 py-0.5 text-[0.6875rem] font-bold uppercase tracking-[0.08em] leading-none ${className}`}>
      {eventTypeLabel(type)}
    </span>
  )
}
