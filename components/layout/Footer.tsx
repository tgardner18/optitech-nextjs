      ...
      <div className="relative flex flex-col lg:flex-row min-h-65" style={{ zIndex: 1 }}>
        ...
        <div className="relative lg:w-[58%] flex flex-col justify-center px-md py-xl lg:px-lg lg:pr-24">
          ...
          <Link
            ...
            style={{ filter: 'drop-shadow(0 6px 24px var(--ot-bloom-brand-faint))' }}
          >
            <Image src={footerLogoSrc} alt={logoAlt} width={444} height={90} className={logoImgClass} />
          </Link>
          {descriptionHtml && (
            <div
              className="relative mt-lg max-w-[46ch] text-[0.9375rem] leading-[1.65] text-fg-muted [&_p]:m-0 [&_p+p]:mt-[0.75em] [&_strong]:font-semibold [&_strong]:text-fg [&_em]:not-italic [&_em]:text-accent [&_a]:text-fg-muted [&_a]:underline [&_a]:decoration-fg/20 [&_a:hover]:text-fg [&_a:hover]:decoration-fg/50 transition-colors duration-150"
        ...
        {links.length > 0 && (
          <div
            ...
            style={{
              width: '2px', left: '58%', transform: 'skewX(-4deg)', transformOrigin: 'top center',
        ...
        {links.length > 0 && (
          <div className="relative lg:w-[42%] flex flex-col justify-center px-md py-xl lg:px-lg border-t border-fg/8 lg:border-t-0">
            {links.length >= 3 && (
              <p className="text-label tracking-label uppercase text-fg-muted/40 font-semibold mb-md">Navigation</p>
            )}
            <nav aria-label="Footer navigation">
              <ul className={`grid grid-cols-1 gap-y-xs ${twoColumnLinks ? 'sm:grid-cols-2 gap-x-xl' : ''}`}>
                {links.map(link => (
                  <li key={link.label}>
                    <Link href={link.href} className="text-label tracking-label uppercase text-fg-muted hover:text-fg transition-colors duration-150 ease-quick">{link.label}</Link>
                  </li>
                ))}
              </ul>
            </nav>
